                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        ...
                        <div className="bg-gray-800 rounded-xl p-6 border border-gray-700 shadow-xl">
                                    ...
                                    <>
                                        ...
                                        {(isAnalyzing || isRemovingBg) && (
                                            <div className="absolute inset-0 bg-black/80 flex flex-col items-center justify-center z-10 backdrop-blur-sm animate-fade-in">
                                                ...
                                                {isRemovingBg ? (
                                                    <>
                                                        <p className="text-green-400 font-bold text-base animate-pulse">✂️ 배경 제거(누끼) 중...</p>
                                                        {elapsedBg > 0 && <span className="text-xs text-gray-400 tabular-nums">{formatElapsed(elapsedBg)}</span>}
                                                    </>
                                                ) : (
                                                    <>
                            ...
                            {/* Remove.bg Tip */}
                            <div className={`mt-3 border rounded-lg p-3 flex items-center gap-3 animate-fade-in ${hasKey ? 'bg-green-900/30 border-green-500/30' : 'bg-amber-900/30 border-amber-500/30'}`}>
                                <span className="text-lg">{hasKey ? '✅' : '💡'}</span>
                                <p className={`text-sm leading-relaxed ${hasKey ? 'text-green-100/90' : 'text-amber-100/90'}`}>
                                    {hasKey ? <><strong>자동 누끼 제거 활성화됨</strong> — 업로드 시 AI가 자동으로 배경을 제거합니다.</> : <><strong>꿀팁:</strong> 배경이 제거된 이미지를 사용하면 AI 인식률이 대폭 상승합니다!</>}
                                </p>
                            </div>
                        </div>
