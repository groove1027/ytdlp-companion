            ...
            <div className="flex-1">
              ...
              <ol className="mt-3 text-sm text-amber-200/80 space-y-2">
                <li className="flex gap-2">
                  ...
                  <span>{getCompanionOsLabel() === 'Windows' && !COMPANION_WINDOWS_AVAILABLE
                    ? <>컴패니언 앱 <span className="text-amber-400 font-medium">Windows 버전 준비 중</span> (현재 macOS만 지원)</>
                    : <>컴패니언 앱을 <a href={getCompanionDownloadUrl()} target="_blank" rel="noopener noreferrer" className="text-cyan-400 underline hover:text-cyan-300 font-medium">여기서 다운로드{getCompanionOsLabel() ? ` (${getCompanionOsLabel()})` : ''}</a>하세요</>
                  }</span>
                </li>
                <li className="flex gap-2">
