...
async function racePipedInvidious(videoId: string): Promise<string | null> {
  ...
  try {
    ...
    console.warn('[Frame] Piped/Invidious 전부 실패');
    return null;
  }
}
...
async function extractVideoFrames(file: File, sourceIndex?: number): Promise<TimedFrame[]> {
  ...
  return Promise.race([
    ...
    new Promise<TimedFrame[]>((resolve) => {
      setTimeout(() => {
        console.warn(`[extractVideoFrames] ${file.name}: ${OVERALL_TIMEOUT_MS}ms 타임아웃 — 프레임 추출 건너뜀`);
        resolve([]);
      }, OVERALL_TIMEOUT_MS);
    }),
  ]);
...
${meta.description.slice(0, 1500)}${meta.description.length > 1500 ? '\n...(이하 생략)' : ''}` +
      ...
      // [FIX #157] 모든 API 호출에 AbortSignal 전달
      ...
      if (videoUri) {
        ...
        try {
          ...
          if (frames.length > 0) {
            text = await analyzeWithFrames(frames, userPrompt, scriptSystem, maxTokens, signal);
          } else {
            const messages: EvolinkChatMessage[] = [
              { role: 'system', content: scriptSystem },
              { role: 'user', content: userPrompt },
            ];
            text = await evolinkChatStream(messages, () => {}, { temperature: 0.5, maxTokens: maxTokens, signal });
          }
        }
