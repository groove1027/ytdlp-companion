...
const VideoAnalysisRoom: React.FC = () => {
  ...
  const handleAnalyze = async (preset: AnalysisPreset, force = false) => {
    ...
    // [FIX #157] 글로벌 타임아웃 — 무한 대기 방지
    // [FIX #189] 다중 영상 시 영상당 +3분 확장 (1개=5분, 2개=8분, 3개=11분)
    const _sourceCount = Math.max(uploadedFiles.length || validYoutubeUrls.length, 1);
    const GLOBAL_TIMEOUT_MS = (5 + Math.max(_sourceCount - 1, 0) * 3) * 60 * 1000;
    const globalTimeout = setTimeout(() => {
      console.warn('[VideoAnalysis] 글로벌 타임아웃 (5분) 도달 — 분석 강제 중단');
      abortCtrl.abort();
    }, GLOBAL_TIMEOUT_MS);
    ...
    try {
      ...
      if (uploadedFiles.length > 0) {
        ...
        if (allFrames.length === 0 && uploadedFiles.length > 0) {
          ...
          try {
            // [FIX #189] 다중 영상: 모든 파일 Cloudinary 업로드
            ...
            // [FIX #189] 추가 영상 v1beta 사전 요약 → inputDesc에 병합 (2번째 영상부터)
            if (uploadedVideoUris.length > 1) {
              const extraSummaries: string[] = [];
              for (let fi = 1; fi < uploadedVideoUris.length; fi++) {
                try {
                  showToast(`추가 영상 ${fi + 1} 사전 분석 중...`, 3000);
                  const summary = await evolinkVideoAnalysisStream(
                    uploadedVideoUris[fi],
                    uploadedFiles[fi]?.type || 'video/mp4',
                    '영상 분석 전문가입니다.',
                    `이 영상의 핵심 내용을 구조적으로 정리해주세요:\n- 주요 장면/시퀀스 (타임코드)\n- 대사 또는 나레이션\n- 화면 구성과 전환\n간결하게 2000자 이내로.`,
                    () => {}, { temperature: 0.3, maxOutputTokens: 4000, signal: abortCtrl.signal },
                  );
                  extraSummaries.push(`[소스 ${fi + 1}] ${uploadedFiles[fi].name}\n${summary}`);
                  console.log(`[VideoAnalysis] 추가 영상 ${fi + 1} 사전 분석 완료`);
                } catch (e) {
                  console.warn(`[VideoAnalysis] 추가 영상 ${fi + 1} 사전 분석 실패:`, e);
                  extraSummaries.push(`[소스 ${fi + 1}] ${uploadedFiles[fi].name} (${((uploadedFiles[fi].size || 0) / 1024 / 1024).toFixed(1)}MB) — 사전 분석 실패`);
                }
              }
              if (extraSummaries.length > 0) {
                inputDesc += `\n\n---\n\n## 추가 영상 분석 결과\n아래 영상의 내용도 반드시 짜집기에 포함하세요.\n\n` + extraSummaries.join('\n\n---\n\n');
              }
            }
          } catch (uploadErr) {
...
${(socialMeta.description || '').slice(0, 1500)}${(socialMeta.description || '').length > 1500 ? '\n...(이하 생략)' : ''}${commentText}`);
      ...
      // 롱폼 감지: 프레임 중 가장 큰 타임코드가 5분(300초) 이상이면 롱폼
        ...
        if (videoUri) {
          try {
            return await evolinkVideoAnalysisStream(
              videoUri, videoMime, scriptSystem, prompt,
      ...
      if (useParallelBatch) {
        ...
        const batchPromises = Array.from({ length: BATCH_COUNT }, (_, bi) => {
          ...
          return callAI(batchPrompt, batchTokens).then(batchText => {
        ...
        if (successes === 0) {
          // 전체 실패 시 에러 체크 — abort 우선
          if (signal.aborted) throw new DOMException('분석이 취소되었습니다.', 'AbortError');
          const firstErr = results.find(r => r.status === 'rejected') as PromiseRejectedResult | undefined;
          throw new Error(firstErr?.reason?.message || '모든 병렬 분석 배치가 실패했습니다.');
        }
        if (successes < BATCH_COUNT) {
          showToast(`${BATCH_COUNT}개 배치 중 ${successes}개 성공 — 일부 버전이 누락될 수 있습니다.`, 5000);
        }
