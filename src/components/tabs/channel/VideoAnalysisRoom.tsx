...
import { buildVideoAnalysisEditTableText, getVideoAnalysisPrimaryText } from '../../../utils/videoAnalysisText';
...
import { DUBBING_TRANSLATION_GUIDELINE } from '../../../data/dubbingTranslationGuideline';
import { S2S_STYLE_CLONING_GUIDELINE } from '../../../data/s2sStyleCloningGuideline';
import { LONG_TO_SHORT_SNACK_GUIDELINE } from '../../../data/longToShortSnackGuideline';
...
const PRESET_DISPLAY_LABEL: Record<AnalysisPreset, string> = {
  tikitaka: '티키타카 편집점',
  snack: '스낵형 편집점',
  condensed: '축약 리캡',
  deep: '심층 분석',
...
const PRESET_AUDIO_HEADER: Record<AnalysisPreset, string> = {
  tikitaka: '오디오 내용',
  snack: '자막 내용',
  condensed: '내레이션',
  deep: '내레이션',
...
const PRESET_MAX_VERSION_COUNT: Record<AnalysisPreset, number> = {
  tikitaka: 10,
  snack: 10,
  condensed: 10,
  deep: 5,
...
const ALWAYS_SHORT_FORM_PRESETS = new Set<AnalysisPreset>(['tikitaka', 'snack', 's2s', 'l2s']);
const DIALOGUE_PRIORITY_PRESETS = new Set<AnalysisPreset>(['snack', 's2s', 'l2s']);
const ORIGINAL_LANGUAGE_COPY_PRESETS = new Set<AnalysisPreset>(['tikitaka', 'dubbing']);

const PRESET_SRT_LABELS: Record<AnalysisPreset, { dialogue: string; effect: string; original?: string }> = {
  tikitaka: { dialogue: '일반자막', effect: '효과자막' },
  snack: { dialogue: '자막', effect: '이원화자막' },
  condensed: { dialogue: '일반자막', effect: '효과자막' },
  deep: { dialogue: '일반자막', effect: '효과자막' },
  shopping: { dialogue: '나레이션', effect: '상품효과' },
  alltts: { dialogue: '일반자막', effect: '효과자막' },
  dubbing: { dialogue: '한국어', effect: '가이드', original: '원어' },
  s2s: { dialogue: '자막', effect: '밈자막' },
  l2s: { dialogue: '하단자막', effect: '강조자막' },
};

function getPresetDisplayLabel(preset: AnalysisPreset): string {
  return PRESET_DISPLAY_LABEL[preset];
}

function getPresetAudioHeader(preset: AnalysisPreset): string {
  return PRESET_AUDIO_HEADER[preset];
}

function getPresetMaxVersionCount(preset: AnalysisPreset): number {
  return PRESET_MAX_VERSION_COUNT[preset];
}

function getPresetSrtLabels(preset?: AnalysisPreset | null): { dialogue: string; effect: string; original?: string } {
  return PRESET_SRT_LABELS[preset || 'snack'];
}

function isPresetAlwaysShortForm(preset?: AnalysisPreset | null): boolean {
  return !!preset && ALWAYS_SHORT_FORM_PRESETS.has(preset);
}

function getPrimarySceneText(scene: SceneRow, preset?: AnalysisPreset): string {
  return getVideoAnalysisPrimaryText(scene, preset);
}

function hasPresetOriginalSrtLayer(preset: AnalysisPreset | null | undefined, scenes: SceneRow[]): boolean {
  return preset === 'dubbing' && scenes.some(scene => !!scene.audioContentOriginal?.trim());
}
...
function extractFallbackVersionBody(content: string): string {
  return unwrapFencedCodeBlocks(
    content
      .replace(/^\s*(?:\*{0,2})(?:제목|title)(?:\*{0,2})[:\s：]+\s*.+$/gim, '')
      .replace(/^\s*(?:\*{0,2})(?:재배치\s*구조|rearrangement)(?:\*{0,2})[:\s：]+\s*.+$/gim, '')
      .replace(/^\s*(?:\*{0,2})(?:컨셉|콘셉트|concept)(?:\*{0,2})[:\s：]+\s*/gim, '')
      .replace(/\n{3,}/g, '\n\n'),
  ).trim();
}
...
function sanitizeParsedVersions(items: VersionItem[]): { versions: VersionItem[]; removedEmptyIds: number[] } {
  const removedEmptyIds: number[] = [];
  const versions = items
    .map((item) => item.scenes.length === 0
      ? {
          ...item,
          concept: extractFallbackVersionBody(item.concept),
        }
      : item)
    .filter((item) => {
      const keep = item.scenes.length > 0 || hasMeaningfulVersionText(`${item.concept}\n${item.rearrangement || ''}`);
      if (!keep) removedEmptyIds.push(item.id);
      return keep;
    });

  return { versions, removedEmptyIds };
}
...
function buildTextOnlyVersionFallback(raw: string): VersionItem[] {
  ...
  if (sections.length > 0) {
    return sections.map(({ id, titleHint, content }) => ({
      ...
      concept: extractFallbackVersionBody(content) || titleHint || '',
      scenes: [],
    }));
  }
    ...
    if (match && parseInt(match[1], 10) <= 10) {
      if (current) {
        fallback.push({
          id: current.id!,
          title: current.title!,
          concept: extractFallbackVersionBody(body.join('\n').trim()),
          scenes: [],
        });
      }
...
function parseVersions(raw: string): VersionItem[] {
  ...
  for (let i = 1; i < altBlocks.length; i += 2) {
    ...
    const parsedScenes = block.split('\n').some(l => /\|\s*\d+\s*\|/.test(l)) ? parseTikitakaTable(block) : [];
    altItems.push({
      id: n,
      title: tMatch?.[1]?.trim().replace(/\*+/g, '') || block.split('\n')[0]?.trim().slice(0, 60) || `버전 ${n}`,
      concept: parsedScenes.length === 0 ? extractFallbackVersionBody(block) : '',
      scenes: parsedScenes,
    });
  }
  if (sanitizeParsedVersions(altItems).versions.length >= 2) return altItems;
  ...
  if (hasShoppingFormat) {
    ...
    for (let i = 1; i < shopBlocks.length; i += 2) {
      ...
      const codeMatch = content.match(/```(?:[a-zA-Z0-9_-]+)?\s*\n([\s\S]*?)```/);
      const script = codeMatch?.[1]?.trim() || extractFallbackVersionBody(content.split('\n').slice(1).join('\n').trim());
      shopItems.push({ id: n, title: titleLine, concept: script, scenes: [] });
    }
    if (sanitizeParsedVersions(shopItems).versions.length >= 2) return shopItems;
  }
...
function extractTaggedSourceIndex(text: string): number | undefined {
...
async function analyzeWithFrames(
  ...
  const maxFrames = 14;
  const framesBySource = new Map<number, TimedFrame[]>();
  frames.forEach((frame) => {
    const sourceIndex = frame.sourceIndex ?? 0;
    if (!framesBySource.has(sourceIndex)) framesBySource.set(sourceIndex, []);
    framesBySource.get(sourceIndex)!.push(frame);
  });
  const sortedSourceEntries = Array.from(framesBySource.entries()).sort((left, right) => left[0] - right[0]);
  const hasMultipleSources = sortedSourceEntries.length > 1;
  const selectedFrames = (() => {
    if (!hasMultipleSources) return pickEvenly(frames, maxFrames);
    const perSourceLimit = Math.max(1, Math.floor(maxFrames / sortedSourceEntries.length));
    const picked: TimedFrame[] = [];
    const pickedKeys = new Set<string>();
    const makeKey = (frame: TimedFrame) => `${frame.sourceIndex ?? 0}:${Math.round(frame.timeSec * 100)}:${frame.url.slice(0, 24)}`;
    sortedSourceEntries.forEach(([, sourceFrames]) => {
      pickEvenly(sourceFrames, Math.min(sourceFrames.length, perSourceLimit)).forEach((frame) => {
    ...
    if (picked.length < maxFrames) {
      const leftovers = sortedSourceEntries
        .flatMap(([, sourceFrames]) => sourceFrames)
        .filter((frame) => !pickedKeys.has(makeKey(frame)));
      pickEvenly(leftovers, maxFrames - picked.length).forEach((frame) => {
        const key = makeKey(frame);
        if (pickedKeys.has(key)) return;
        pickedKeys.add(key);
        picked.push(frame);
    ...
    return picked
      .sort((left, right) => {
        const sourceDelta = (left.sourceIndex ?? 0) - (right.sourceIndex ?? 0);
  ...
  const frameData = selectedFrames.map(f => {
    ...
    return {
      base64,
      mimeType,
      label: hasMultipleSources
        ? `[프레임 소스 ${String((f.sourceIndex ?? 0) + 1).padStart(2, '0')} · ${(f.sourceFileName || '영상').slice(0, 30)} · ${formatTimeSec(f.timeSec)}]`
        : `[프레임 ${formatTimeSec(f.timeSec)}]`,
    };
  ...
  const enrichedPrompt = hasMultipleSources
    ? `${userPrompt}\n\n[아래는 ${sortedSourceEntries.length}개 서로 다른 영상 소스에서 추출한 ${frameData.length}개 프레임입니다. 프레임 라벨의 소스 번호를 기준으로 모든 소스를 빠짐없이 반영하고, 특정 한 소스만 요약하지 말고 여러 소스를 비교/조합하여 분석해주세요.]`
...
/** SRT 레이어 타입: dialogue=주 자막, effect=보조 자막, combined=통합, original=원어 자막 */
type SrtLayer = 'dialogue' | 'effect' | 'combined' | 'original';
...
function generateSrt(
  ...
  const getLayerText = (scene: SceneRow): string => {
    switch (layer) {
      case 'dialogue':
        return applyBreak(getPrimarySceneText(scene, preset));
      case 'original':
        return applyBreak(scene.audioContentOriginal || '');
      case 'effect':
        return applyBreak(scene.effectSub || '');
      case 'combined':
      default:
        if (preset === 'dubbing' && scene.audioContentOriginal?.trim()) {
          return `${applyBreak(scene.audioContentOriginal)}\n${applyBreak(getPrimarySceneText(scene, preset))}`;
        }
...
function generateSyncedSrt(
  ...
  const getLayerText = (scene: SceneRow): string => {
    switch (layer) {
      case 'dialogue': return applyBreak(getPrimarySceneText(scene, preset));
      case 'original': return applyBreak(scene.audioContentOriginal || '');
...
function generateAnalysisHtml(
  ...
  const presetLabel = getPresetDisplayLabel(preset);
  ...
  const versionsHtml = versions.map((v, vi) => {
    ...
    if (v.scenes.length > 0) {
      const headerCells = isTk
        ? `<th>#</th><th>모드</th><th>${getPresetAudioHeader(preset)}</th><th>효과자막</th><th>예상시간</th><th>비디오 화면 지시</th><th>타임코드</th>`
      ...
      const bodyRows = v.scenes.map((s, sIdx) => {
        ...
        if (isTk) {
          return `<tr>
            ...
            <td class="audio-cell">${escHtml(getPrimarySceneText(s, preset) || '-')}</td>
...
const DUBBING_SCRIPT_SYSTEM = DUBBING_TRANSLATION_GUIDELINE;

const S2S_SCRIPT_SYSTEM = S2S_STYLE_CLONING_GUIDELINE;

const L2S_SCRIPT_SYSTEM = LONG_TO_SHORT_SNACK_GUIDELINE;
...
const PRESET_INFO: Record<AnalysisPreset, { label: string; description: string; color: string }> = {
  ...
  dubbing: { label: '더빙 번역', description: '해외 영상 → 최상급 한국어 더빙 대본 — 정보 밀도 동기화 + 성우 호흡 최적화 + SRT 자막 + 학습 포인트', color: 'rose' },
  s2s: { label: '숏투숏', description: '1줄 TTS + 무음 밈 자막 — 이원화 숏폼 변환. 초압축 오디오 훅 + 밈 범벅 자막 융단폭격', color: 'orange' },
  l2s: { label: '롱투숏', description: '롱폼 → 샌드위치 숏폼 변환 — 상단 고정 타이틀 + 중앙 영상 + 하단 동적 자막. 영웅 서사 4단계 구조', color: 'lime' },
};
...
function buildMultiSourcePromptHeader(sourceCount: number, sourceTypeLabel: string): string {
  return `## 다중 영상 짜집기 분석 (${sourceCount}개 소스)
아래 ${sourceCount}개 ${sourceTypeLabel} 소스를 각각 독립된 원본으로 인식하세요.
- 특정 한 소스만 요약하지 말고, 모든 소스를 검토한 뒤 편집표를 설계하세요.
- 각 버전은 가능한 한 여러 소스를 비교/조합하여 구성하세요.
- 소스 번호([소스 01], [소스 02] ...)를 유지한 채 어떤 소스를 활용했는지 비디오 화면 지시에 반영하세요.
...
interface DubbingSegmentGuide {
  effectiveDurationSec: number;
  segmentCount: number;
  totalLengthLabel: string;
}

const countTranscriptSegmentsFromInputDesc = (inputDesc: string): number => {
  const timedCueCount = inputDesc.match(/^\[(?:\d+:)?\d{2}:\d{2}\.\d{3}~(?:\d+:)?\d{2}:\d{2}\.\d{3}\]/gm)?.length || 0;
  const diarizedCueCount = inputDesc.match(/^\[(?:speaker_[^\s\]]+|[^\]\s]+)\s+\d{1,2}:\d{2}(?:\.\d+)?~\d{1,2}:\d{2}(?:\.\d+)?\]/gm)?.length || 0;
  return Math.max(timedCueCount, diarizedCueCount);
};

const buildDubbingSegmentGuide = (
  inputDesc: string,
  videoDurationSec: number,
): DubbingSegmentGuide | null => {
  const effectiveDurationSec = resolveEffectiveVideoDurationSec(inputDesc, videoDurationSec);
  const transcriptSegmentCount = countTranscriptSegmentsFromInputDesc(inputDesc);
  const segmentCount = transcriptSegmentCount > 0
    ? transcriptSegmentCount
    : effectiveDurationSec > 0
      ? clampNumber(Math.round(effectiveDurationSec / 3.5), 8, 160)
      : 0;
  ...
  return {
    ...
    segmentCount,
    totalLengthLabel: effectiveDurationSec > 0
      ? `${Math.round(effectiveDurationSec * 0.95)}~${Math.round(effectiveDurationSec * 1.05)}초`
      : '원본과 동일한 총 길이',
  };
};
...
const buildUserMessage = (
  ...
  const dubbingSegmentGuide = buildDubbingSegmentGuide(inputDesc, videoDurationSec);
  const s2sTargetDuration = targetDuration === 0 ? 20 : Math.min(targetDuration, 20);
  const s2sTargetRows = sourceCutCount > 0
    ? clampNumber(Math.min(sourceCutCount, 14), 8, 14)
    : clampNumber(Math.round(s2sTargetDuration / 1.8), 8, 14);
  const s2sRowRule = `10. **총 길이는 반드시 ${Math.max(12, s2sTargetDuration - 4)}~${s2sTargetDuration}초 이내.** 버전당 최소 ${Math.max(6, s2sTargetRows - 2)}개, 권장 ${s2sTargetRows}개, 최대 ${Math.min(14, s2sTargetRows + 2)}개 컷으로 설계하고, 1번 자막만 TTS 훅으로 사용하라.`;
  const l2sTargetRows = targetDuration === 30 ? 14 : targetDuration === 45 ? 18 : 22;
  const l2sMinRows = targetDuration === 30 ? 12 : targetDuration === 45 ? 16 : 20;
  const l2sMaxRows = targetDuration === 30 ? 16 : targetDuration === 45 ? 20 : 25;
  const l2sTargetLengthLabel = targetDuration === 30 ? '28~35초' : targetDuration === 45 ? '40~50초' : '55~65초';
...
## 지시 사항
위 해외 원본 영상/링크/대본을 분석하여, 시스템 프롬프트의 **더빙 AI용 절대 지침서 v5.6**을 문장 하나도 축약하지 말고 실행하세요.

### 🚨 최우선 규칙
- **해외 언어 원본 전제 절대 유지:** 한국어 원본으로 오판하지 마라.
- **원문 누락 절대 금지:** 사용자가 제공한 링크, 대본, 영상의 정보량을 한 글자도 빠뜨리지 마라.
- **세그먼트 순서 고정:** 타임코드와 원본 세그먼트 순서를 절대 뒤섞지 마라.
- **정보 밀도 동기화:** 각 세그먼트의 음절/초를 한국어 더빙에서도 최대한 동일하게 재현하라.
- **성우 호흡 최적화:** 직역 대신 실제 녹음 가능한 최상급 한국어 대본으로 다듬어라.
- **총 길이 유지:** 전체 분량은 ${dubbingSegmentGuide?.totalLengthLabel || '원본과 동일한 총 길이'}로 유지하고, 원본 세그먼트 약 ${dubbingSegmentGuide?.segmentCount || 'N'}개를 기준으로 표를 설계하라.
- **각 버전은 번역 전략만 달라야 한다.** 원문 정보량, 시간축, 세그먼트 순서는 그대로 유지.

### ⚠️ 절대 규칙 (위반 시 전체 재작성)
1. 출력은 반드시 **번역 브리핑 → 퍼포먼스 청사진 표 → 최종 번역 3단 표 → 한국어/원어 SRT → 학습 포인트** 순서를 따른다.
2. 각 VERSION마다 위 5개 섹션을 모두 완성한 뒤, 마지막에 **[앱 파싱용 7열 마스터 편집 테이블]**을 추가하라.
3. **앱 파싱용 표도 원본 세그먼트 기준으로 빠짐없이 작성**하고, 각 행의 타임코드는 **MM:SS.ms~MM:SS.ms** 형식으로 적어라.
4. 앱 파싱용 표의 **오디오 내용 열은 반드시 \`원어 ⟶ 한국어 더빙\` 형식**으로 작성하여 원문과 번역이 함께 남도록 하라.
5. 앱 파싱용 표의 모드는 전부 **[N]** 으로 작성하고, 효과자막 열에는 더빙 가이드/연기 메모를 넣어라.
6. 한국어 SRT와 원어 SRT는 각각 별도 코드 블록으로 출력하라.
7. 학습 포인트는 **필수 단어 3개 + 핵심 회화 표현 3개**를 한글 발음 표기와 함께 적어라.
...
[앱 파싱용 7열 마스터 편집 테이블]
| 순서 | 모드 | 오디오 내용 (원어 ⟶ 한국어 더빙) | 효과자막 | 예상 시간 | 비디오 화면 지시 | 타임코드 소스 (MM:SS.ms) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | [N] | [원어 문장] ⟶ [한국어 더빙 문장] | [호흡/톤 가이드] | 2.8초 | 실제 화면 묘사 | 00:03.200~00:06.000 |
...
(이 패턴으로 ---VERSION ${versionCount}--- 까지 총 ${versionCount}개)`, versionStart, versionCount);
  }
...
## 지시 사항
위 영상을 시스템 프롬프트의 **이원화 숏폼 템플릿 v7**에 따라, **1줄 TTS + 나머지 무음 밈 자막** 구조의 숏폼으로 재가공하세요.

### 🚨 최우선 규칙
- **응답 맨 앞에 제목 10개 코드 블록 1개**를 먼저 출력하라.
- **총 길이 20초 초과 금지.** 사용자가 더 길게 요청해도 20초 안에서 해결하라.
- **1번 자막만 TTS 오디오 훅**이고, 2번부터는 전부 무음 자막이다.
- **건조한 첫 문장 + 밈 범벅 무음 자막**의 이원화 구조를 절대 깨지 마라.
- 장면과 무관한 드립 금지. 각 자막은 실제 화면 내용과 정확히 맞아야 한다.
- 원본 순서를 꼭 따를 필요는 없으며, 가장 강한 훅을 1번 컷으로 당겨라.

### ⚠️ 절대 규칙 (위반 시 전체 재작성)
1. 각 VERSION마다 반드시 **Video Spec → SRT 코드 블록 → 컷 타임라인 표 → 앱 파싱용 7열 마스터 편집 테이블** 순서로 출력하라.
2. 앱 파싱용 표에서 **1행만 [N]**, 2행부터는 **[A] 또는 [S]** 로 작성하라.
3. 앱 파싱용 표의 오디오 내용 열에는 실제 노출될 자막 텍스트를 넣고, 효과자막 열에는 중앙 밈/리액션 텍스트를 넣어라.
4. 타임코드 소스는 **MM:SS.ms~MM:SS.ms** 형식으로 합쳐 적고, 각 컷은 대체로 0.5~2.5초 안에 끝내라.
5. 문장 끝은 \`~다는데\` 또는 \`~라는데\` 훅 1줄 후, 나머지는 명사형/감탄사/밈 종결만 허용한다.
6. 10개 버전은 서로 다른 훅/밈 조합/편집 템포로 차별화하라.
${s2sRowRule}
...
제목: [제목 10개 중 1개]
컨셉: [훅 전략 + 밈 톤 1~2줄]
재배치 구조: [예: ④클라이맥스 → ②리액션 → ①발단]

Video Spec: [Timecode + Framerate + Frame Interval]
...
| 컷 번호 | 쇼츠 타임라인 | 원본 FPS | 원본 In 타임코드 | 원본 Out 타임코드 | 컷 길이 | 하단 동적 자막 | 중앙 영상 화면 연출 지시 |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |

[앱 파싱용 7열 마스터 편집 테이블]
| 순서 | 모드 | 오디오 내용 (1행=TTS, 나머지=무음 자막) | 효과자막 | 예상 시간 | 비디오 화면 지시 | 타임코드 소스 (MM:SS.ms) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | [N] | [건조한 TTS 훅] | [중앙 밈] | 1.2초 | 실제 장면 묘사 | 00:03.200~00:04.400 |
| 2 | [A] | [무음 자막] | [리액션 밈] | 1.0초 | 실제 장면 묘사 | 00:08.100~00:09.100 |
...
## 지시 사항
위 롱폼 영상을 시스템 프롬프트의 **샌드위치 쇼츠 지침서 v4.0**에 따라, **상단 고정 타이틀 + 중앙 영상 + 하단 동적 자막** 구조의 롱투숏으로 재구성하세요.

### 🚨 최우선 규칙
- **영웅 서사 4단계**를 반드시 따른다: 훅/편견 → 시련/무명 → 독기/노력 → 반전/카타르시스.
- **상단 고정 타이틀 1줄**을 먼저 정하고, 영상 끝까지 유지된다는 전제로 설계하라.
- **하단 동적 자막 20~25개**를 기본으로 작성하되, 사용자가 더 짧은 목표 시간을 고르면 그 길이에 맞춰 줄 수를 줄여라.
- 말투는 **음/함/임 체**를 유지하고, 설명충/존댓말/느린 편집을 금지한다.
- 훅 컷은 강하게 당겨오되, 전체 서사는 4단계 감정 곡선이 분명해야 한다.

### ⚠️ 절대 규칙 (위반 시 전체 재작성)
1. 각 VERSION마다 반드시 **상단 고정 타이틀 → 샌드위치 자막 리스트 → 컷 편집 타임라인 표 → SRT 코드 블록 → 앱 파싱용 7열 마스터 편집 테이블** 순서로 출력하라.
2. 컷 편집 타임라인 표에는 **원본 FPS, 원본 In/Out 타임코드, 컷 길이, 하단 동적 자막, 중앙 영상 연출 지시**를 모두 포함하라.
3. 앱 파싱용 7열 표는 **최소 ${l2sMinRows}행, 권장 ${l2sTargetRows}행, 최대 ${l2sMaxRows}행**, 총 길이 **${l2sTargetLengthLabel}** 기준으로 작성하라.
4. 앱 파싱용 표의 오디오 내용 열에는 **하단 동적 자막**, 효과자막 열에는 **강조 단어/상단 연출 메모**, 비디오 화면 지시에는 **중앙 영상 연출**을 적어라.
5. 타임코드 소스는 **MM:SS.ms~MM:SS.ms** 형식으로 합쳐 적고, 실제 원본 장면만 사용하라.
6. 10개 버전은 서로 다른 훅 문장, 타이틀, 편집 호흡으로 차별화하되 같은 영웅 서사 4단계를 유지하라.

### 출력 포맷

---VERSION 1---
제목: [상단 고정 타이틀]
컨셉: [훅 포지션 + 영웅 서사 운용 방식 1~2줄]
재배치 구조: [예: ④증명 → ②시련 → ③노력 → ④카타르시스]
...
[앱 파싱용 7열 마스터 편집 테이블]
| 순서 | 모드 | 오디오 내용 (하단 동적 자막) | 효과자막 | 예상 시간 | 비디오 화면 지시 | 타임코드 소스 (MM:SS.ms) |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| 1 | [A] | [하단 자막 1] | [핵심 하이라이트] | 2.2초 | 실제 중앙 영상 연출 지시 | 00:15.200~00:17.400 |
...
const VideoAnalysisRoom: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const presetMaxVersionCount = getPresetMaxVersionCount(selectedPreset);
    if (versionCount > presetMaxVersionCount) {
      setVersionCount(presetMaxVersionCount);
    }
  }, [selectedPreset, versionCount, setVersionCount]);
  ...
  const handleAnalyze = async (
    ...
    const scriptSystem = preset === 'tikitaka' ? TIKITAKA_SCRIPT_SYSTEM
      ...
      : preset === 'dubbing' ? DUBBING_SCRIPT_SYSTEM
      : preset === 's2s' ? S2S_SCRIPT_SYSTEM
      : preset === 'l2s' ? L2S_SCRIPT_SYSTEM
      : SNACK_SCRIPT_SYSTEM;
      ...
      if (cachedSourcePrep) {
        ...
        for (let fi = 0; fi < frameResults.length; fi++) {
          ...
          if (r.status === 'fulfilled' && r.value.length > 0) {
            ...
            frameCount = r.value.length;
          }
          fileDescs.push(`[소스 ${String(fi + 1).padStart(2, '0')}] ${uploadedFiles[fi].name} (${((uploadedFiles[fi].size || 0) / 1024 / 1024).toFixed(1)}MB, 추출 프레임 ${frameCount}개)`);
        }
        frames = allFrames;
        ...
        if (isMultiSource) {
          inputDesc = buildMultiSourcePromptHeader(uploadedFiles.length, '업로드 영상') + fileDescs.join('\n');
        } else {
        ...
        if (firstIsYouTube) {
          ...
          const normalizedYoutubeUris = urls
          ...
          if (normalizedYoutubeUris.length > 0) {
            ...
            // [FIX #1028] 다중 YouTube 분석 시 첫 영상만 전달되던 문제 수정
            allVideoUris = normalizedYoutubeUris;
            allVideoMimes = normalizedYoutubeUris.map(() => 'video/mp4');
            videoUri = normalizedYoutubeUris[0];
            videoMime = allVideoMimes[0];
          }

          // [FIX #perf] 메타데이터 + 타임코드 보존 자막을 병렬로 수집
          // 타임드 자막은 AI에게 실제 타임코드를 제공하여 정확도 향상 + 화자분리 대체
          const timedTranscriptPromise = !isMultiSource && primaryVid
...
- **URL**: ${url.trim()}`);
          ...
          if (isMultiSource) {
            inputDesc = buildMultiSourcePromptHeader(urls.length, 'YouTube 영상') + descs.join('\n\n---\n\n');
          } else {
...
${(socialMeta.description || '').slice(0, 1500)}${(socialMeta.description || '').length > 1500 ? '\n...(이하 생략)' : ''}`
          ...
          if (isMultiSource) {
            inputDesc = buildMultiSourcePromptHeader(urls.length, `${platformLabel} 영상`) + descs.join('\n\n---\n\n');
          } else {
      ...
      const diarizePresets = ['tikitaka', 'condensed', 'snack', 'alltts', 'dubbing', 's2s', 'l2s'];
      ...
      setRealStep(2);
      const presetLabel = PRESET_INFO[preset]?.label || getPresetDisplayLabel(preset);
      setStepDetail(
        hasV1betaVideo
      ...
      const isHeavyPresetForTimeout = preset === 'deep' || preset === 'alltts' || preset === 'dubbing';
...
- 결과물 상단에 "⚠️ 이 분석은 영상을 직접 보지 못하고 제목/설명만으로 생성되었습니다. 정확도가 낮을 수 있습니다."를 반드시 포함`;
      ...
      const fixedOverheadTokens: Record<string, number> = {
        ...
        dubbing: 6500, s2s: 1000, l2s: 1800,
      };
      ...
      const minTokensCap = preset === 'deep' ? 25000
        : preset === 'alltts' ? 22000
        : preset === 'dubbing' ? 18000
        : preset === 'l2s' ? 16000
        : 14000;
      ...
      const runLazyDiarizationOnce = (): Promise<void> => {
        if (isMultiSource) return Promise.resolve();
      ...
      if (removedEmptyVersionIds.size > 0) {
        const removedLabel = Array.from(removedEmptyVersionIds).sort((a, b) => a - b).join(', ');
  ...
  const handleSendToEditRoom = useCallback(async (v: VersionItem | null) => {
    ...
    try {
      ...
      const versionText = buildVideoAnalysisEditTableText(targetVersion, selectedPreset || undefined);
  ...
  const handleCopyVersion = useCallback(async (v: VersionItem, mode: 'tts' | 'original' | 'all') => {
    ...
    if (mode === 'tts') {
      ...
      if (preset === 'snack' || preset === 'l2s') {
        ...
        // 숏투숏: 첫 줄 TTS 훅만 추출
        const ttsLine = v.scenes.find(s => s.mode.includes('N'));
        const fallbackScene = ttsLine || v.scenes[0];
        text = fallbackScene ? stripForTts(getPrimarySceneText(fallbackScene, preset)) : '';
      } else if (preset === 'dubbing') {
        // 더빙 번역: 한국어 더빙 대본만 추출
        text = v.scenes
      ...
      // 오리지널 대사: 프리셋별 최적화
      if (preset && ORIGINAL_LANGUAGE_COPY_PRESETS.has(preset) && shouldShowOriginalLanguageControls(v)) {
        // 티키타카 해외 영상: 원어 대사 + 한국어 번역 쌍으로 복사
        const langBadge = getOriginalLanguageBadge(v);
        text = v.scenes
          .map(s => {
            const orig = (s.audioContentOriginal || '').trim();
            const kr = getPrimarySceneText(s, preset).trim();
            return orig ? `[${langBadge}] ${orig}\n[KR] ${kr}` : kr;
          })
          .filter(line => line)
          .join('\n\n');
      } else if (preset === 'snack' || preset === 's2s' || preset === 'l2s') {
      ...
      if (preset === 'deep') {
        ...
        const scenesText = v.scenes.map(s =>
          `[${s.cutNum}] ${s.mode} | ${getPrimarySceneText(s, preset)} | 효과자막: ${s.effectSub || '-'} | ${s.duration} | ${s.videoDirection} | ${s.timecodeSource}`
        ).join('\n');
        text = `제목: ${v.title}\n컨셉: ${v.concept}\n\n${scenesText}`;
      }
    }
  ...
  const handleDownloadSrt = useCallback(async (v: VersionItem) => {
    ...
    const hasOriginalLayer = hasPresetOriginalSrtLayer(selectedPreset, v.scenes);
    const totalDur = v.scenes.reduce((acc, s) => acc + parseDuration(s.duration), 0);
    const isShortForm = selectedPreset === 'deep' ? false
      : isPresetAlwaysShortForm(selectedPreset) ? true
      : totalDur <= 90;
    const { dialogue: dlgLabel, effect: fxLabel, original: originalLabel } = getPresetSrtLabels(selectedPreset);
    const zipLayerLabel = [dlgLabel, hasOriginalLayer ? originalLabel : '', hasEffectSub ? fxLabel : '']
      .filter(Boolean)
      .join(' + ');
  ...
  const ESTIMATED_TOTAL_SEC = useMemo(() => {
    ...
    const vc = selectedPreset ? Math.min(rawVc, getPresetMaxVersionCount(selectedPreset)) : rawVc;
  ...
  useEffect(() => {
    ...
    const isHeavyPresetForFailsafe = selectedPreset === 'deep' || selectedPreset === 'alltts' || selectedPreset === 'dubbing';
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gray-800/50 rounded-xl border border-gray-700 p-6">
        ...
        <div className="flex items-center gap-3 mt-3">
          ...
          <div className="flex bg-gray-900/70 rounded-lg border border-gray-600/50 p-0.5">
            {(selectedPreset === 'dubbing' ? [1, 3] : [1, 3, 5, 10]).map(n => (
              <button
                key={n} type="button"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mt-4">
          {(Object.entries(PRESET_INFO) as [AnalysisPreset, typeof PRESET_INFO['tikitaka']][]).map(([key, info]) => {
            ...
            const cMap: Record<string, { bg: string; border: string; text: string; hover: string }> = {
              ...
              cyan: { bg: 'bg-cyan-600/10', border: 'border-cyan-500/30', text: 'text-cyan-400', hover: 'hover:bg-cyan-600/20' },
              pink: { bg: 'bg-pink-600/10', border: 'border-pink-500/30', text: 'text-pink-400', hover: 'hover:bg-pink-600/20' },
              rose: { bg: 'bg-rose-600/10', border: 'border-rose-500/30', text: 'text-rose-400', hover: 'hover:bg-rose-600/20' },
              orange: { bg: 'bg-orange-600/10', border: 'border-orange-500/30', text: 'text-orange-400', hover: 'hover:bg-orange-600/20' },
              lime: { bg: 'bg-lime-600/10', border: 'border-lime-500/30', text: 'text-lime-400', hover: 'hover:bg-lime-600/20' },
                          ...
                          <div className="relative" data-copy-menu>
                            ...
                            {copyMenuVersionId === v.id && (() => {
                              const ttsLabel = selectedPreset === 'snack' ? 'TTS용 자막 복사'
                                : selectedPreset === 's2s' ? '1줄 TTS 복사'
                                : selectedPreset === 'l2s' ? '하단 자막 복사'
                                : selectedPreset === 'dubbing' ? '더빙 대본 복사'
                              ...
                              const ttsDesc = selectedPreset === 'snack' ? '자막에서 기호 제거'
                                : selectedPreset === 's2s' ? '첫 줄 오디오 훅만 추출'
                                : selectedPreset === 'l2s' ? '하단 자막만 추출'
                                : selectedPreset === 'dubbing' ? '한국어 더빙 대본만 복사'
                              ...
                              const langBadge = getOriginalLanguageBadge(v);
                              const origLabel = selectedPreset === 'snack' || selectedPreset === 's2s' || selectedPreset === 'l2s'
                                ? '자막 원문 복사'
                                : selectedPreset === 'shopping' ? '나레이션+효과 복사'
                                : ((selectedPreset === 'tikitaka' || selectedPreset === 'dubbing') && hasOriginalScenes) ? '원어+한국어 대사 복사'
                                : selectedPreset === 'deep' ? '나레이션 원문 복사'
                                : '오리지널 대사 복사';
                              const origDesc = selectedPreset === 'snack' || selectedPreset === 's2s' || selectedPreset === 'l2s'
                                ? '자막+효과자막 포함'
                                : selectedPreset === 'shopping' ? '상품 효과자막 포함'
                                : ((selectedPreset === 'tikitaka' || selectedPreset === 'dubbing') && hasOriginalScenes) ? `${langBadge}+KR 쌍`
                                : '원본 대사 그대로';
                              const allLabel = selectedPreset === 'deep' ? '분석 보고서 복사'
                                : selectedPreset === 'shopping' ? '쇼핑 대본 전체 복사'
                                : selectedPreset === 'dubbing' ? '더빙 편집표 복사'
                                : selectedPreset === 's2s' ? '숏투숏 전체 복사'
                                : selectedPreset === 'l2s' ? '롱투숏 전체 복사'
                                : '모두 복사';
                              const allDesc = selectedPreset === 'deep' ? '나레이션+화면지시+효과'
                                : selectedPreset === 'shopping' ? '나레이션+효과+타임코드'
                                : selectedPreset === 'dubbing' ? '번역/가이드/타임코드 포함'
                                : '편집표 전체 (모드/효과/타임코드)';
                              return (
                            ...
                            <button type="button" onClick={() => {
                              ...
                              const newLines = v.scenes
                                .flatMap((s, sceneIndex) => {
                                  const sceneText = getPrimarySceneText(s, selectedPreset || undefined);
                                  if (!sceneText) return [];
                      ...
                      {hasScenes && (() => {
                        ...
                        const timelineData = rows.map(r => {
                          ...
                          return {
                            ...
                            mode: r.mode,
                            audioContent: getPrimarySceneText(r, selectedPreset || undefined),
                          ...
                          <table className="w-full text-xs border-collapse">
                            <thead>
                              <tr className="border-b border-gray-700">
                                ...
                                <th className="py-2 px-2 text-left text-gray-500 font-bold">{selectedPreset ? getPresetAudioHeader(selectedPreset) : '내레이션'}</th>
                              ...
                              {v.scenes.map((scene, si) => (
                                <tr key={scene.cutNum} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                                      ...
                                      displayLangMode === 'original' ? (
                                        ...
                                        <div className="space-y-1">
                                          ...
                                          <div className="text-gray-200 leading-relaxed">
                                            ...
                                            {getPrimarySceneText(scene, selectedPreset || undefined)}
                                          </div>
                                        </div>
                                      ) : (
                                        <span>{getPrimarySceneText(scene, selectedPreset || undefined)}</span>
                                      )
                                    ) : (
                                      <span>{getPrimarySceneText(scene, selectedPreset || undefined) || '-'}</span>
                                    )}
                                  ...
                                  {thumbnails.length > 0 && (() => {
                                    ...
                                    // [FIX] 다중 영상: "[소스 N]" / "[S-01]" 태그 모두 지원
                                    const sceneSourceIdx = extractTaggedSourceIndex(tc);
