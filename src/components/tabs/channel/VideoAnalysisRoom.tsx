...
## 4. 출력 형식 (Output Format) — 7열 마스터 편집 테이블
*반드시 아래 형식을 지켜서 출력하시오. 컷 순서는 임팩트 순으로 완전히 뒤섞인 상태여야 합니다!*

각 버전은 ---VERSION N--- 구분자로 분리하고, 고유한 후킹 전략, 톤, 편집 방향으로 차별화합니다.

**테이블 구조 (7열):**
| 순서 | 모드 | 오디오 내용 | 효과자막 | 예상 시간 | 비디오 화면 지시 | 타임코드 소스 |

**모드 규칙:**
- [N]: 내레이션 (AI 나레이션 ON, 원본 소리 MUTE) — 하단 자막(16자+이모지)을 오디오 내용에 포함
- [S]: 현장음-대사 (원본 대사 ON, 내레이션 STOP)
- [A]: 현장음-액션 (원본 현장음 ON, 대사 아닌 소리)

**필수:**
- 효과자막: 예능형 텍스트 2~8자 (💥쾅!, 😳동공지진, [갑분싸])
- 예상 시간: X.X초 형식 (내레이션: 한국어 4글자/초)
- 타임코드 소스: MM:SS 형식, 원본 영상의 실제 위치
...
2. 모드는 **[N](AI 내레이션), [S](원본 대사), [S-내레이션](원본 나레이션), [A](현장음-액션)** 중 하나를 사용.
   - **[N]** = AI가 새로 창작하는 TTS 내레이션 (원본에 없는 음성)
   - **[S]** = 영상 속 인물이 직접 말하는 대사·발언 (대화, 인터뷰, 예능 토크 등)
   - **[S-내레이션]** = 원본 영상에 이미 존재하는 나레이터 음성 (보이스오버, 해설). TTS로 추정되면 **[S-내레이션/TTS추정]**으로 표기.
   - **[A]** = 대사가 아닌 현장음·앰비언스 (액션, 효과음, 한숨, 웃음 등)
...
11. **오디오 소스 사전 분류 필수:** 편집 테이블 작성 전, 원본 오디오를 '인물 대사 vs 나레이션 vs 비음성'으로 사전 분류하고, 나레이션이 TTS인지 인간인지 감지하라.
...
| 4 | [S-내레이션] | (원본 나레이션) "기존 나레이터의 해설" | [효과자막] | 3.0초 | (1) [원본 보이스오버] B-Roll 장면 (3.0초) | 00:22.500 |
...
4. 모드는 **[N](AI 내레이션), [S](원본 대사), [S-내레이션](원본 나레이션), [A](현장음-액션)** 중 하나를 사용. 원본 영상에 나레이터 음성이 있으면 반드시 **[S-내레이션]**으로 구별하라.
...
| 2 | [S] | (인물) "원본 대사 또는 리액션" | [동공지진] | 2.0초 | 대사 인물 클로즈업 | 원본 00:45 |
| 3 | [N] | (내레이션) "상황 설명 + 하단자막 이모지" | [갑분싸] | 2.5초 | 화면 전환 효과 묘사 | 원본 01:30 |
| 4 | [S-내레이션] | (원본 나레이션) "기존 나레이터 해설" | [분위기↑] | 3.0초 | B-Roll 장면 | 원본 03:10 |
                  ...
                  {isExp && (
                    <div className="px-4 pb-4 space-y-3">
                      ...
                      {hasScenes && (() => {
                        ...
                        // 모드별 비율 파이 데이터 (4-Mode Advanced System)
                        const modeDist = [
                          { name: 'AI 내레이션[N]', value: rows.filter(r => extractModeKey(r.mode) === 'N').length, fill: '#3b82f6' },
                          { name: '원본 대사[S]', value: rows.filter(r => extractModeKey(r.mode) === 'S').length, fill: '#10b981' },
                          { name: '원본 나레이션[S-내레이션]', value: rows.filter(r => extractModeKey(r.mode) === 'SN').length, fill: '#06b6d4' },
                          { name: '현장음-액션[A]', value: rows.filter(r => extractModeKey(r.mode) === 'A').length, fill: '#f59e0b' },
                        ].filter(d => d.value > 0);
