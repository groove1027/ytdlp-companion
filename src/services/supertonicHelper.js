/**
 * Supertonic 2 TTS Helper — Vendored from supertone-inc/supertonic
...
export async function loadVoiceStyle(voiceStylePaths, verbose = false) {
    ...
    const firstResponse = await monitoredFetch(voiceStylePaths[0]);
    if (!firstResponse.ok) {
        throw new Error(`Failed to load voice style ${voiceStylePaths[0]}: ${firstResponse.status} ${firstResponse.statusText}`);
    }
    ...
    for (let i = 0; i < bsz; i++) {
        const response = await monitoredFetch(voiceStylePaths[i]);
        if (!response.ok) {
            throw new Error(`Failed to load voice style ${voiceStylePaths[i]}: ${response.status} ${response.statusText}`);
        }
...
export async function loadCfgs(onnxDir) {
    const response = await monitoredFetch(`${onnxDir}/tts.json`);
    if (!response.ok) {
        throw new Error(`Failed to load TTS config from ${onnxDir}/tts.json: ${response.status} ${response.statusText}`);
    }
...
export async function loadTextProcessor(onnxDir) {
    const response = await monitoredFetch(`${onnxDir}/unicode_indexer.json`);
    if (!response.ok) {
        throw new Error(`Failed to load unicode indexer from ${onnxDir}/unicode_indexer.json: ${response.status} ${response.statusText}`);
    }
