...
- [x] **#245 Evolink 429 Rate Limit 쿨다운 + 진행률 UI 정직화** — 근본 원인: Evolink 연속 호출(분석→파싱→엔티티) 시 계정 레이트 리밋 초과, 각 엔드포인트에서 3회 재시도(~21초)씩 낭비. 수정: (1) Evolink 429 감지 시 60초 쿨다운 마킹 → 이후 호출은 Evolink 전체 스킵, 즉시 Kie 폴백 (2) Smart Routing 내 429 재시도 3→1회 축소 (3) 진행률 UI 마지막 단계(품질검수) 분석 완료 전까지 amber 유지 (거짓 완료 방지) (4) 90초+ 메시지 "거의 완료"→"서버 응답 대기" 정직한 안내 (geminiProxy.ts, evolinkService.ts, SetupPanel.tsx, 2026-03-14)
