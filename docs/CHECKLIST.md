...
- [x] #189 다중 영상 분석 실패 수정 — 프레임 추출 타임아웃 시 부분 프레임 반환(빈 배열 대신), 다중 영상 Cloudinary 폴백 시 모든 파일 업로드+추가 영상 v1beta 사전 요약→프롬프트 병합, 글로벌 타임아웃 다중 영상 확장(영상당 +3분), 중단 시 부분 결과 경고 토스트 (VideoAnalysisRoom.tsx, 2026-03-13)
