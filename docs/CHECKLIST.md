...
- [x] **2026-04-12 — TTS/사운드 버그 10건 + 간단 기능 3건 묶음 수정 (#689 #695 #720 #728 #773 #817 #844 #851 #861 #880 #887 #1006 #1053)**
  ...
  - `src/services/audioStorageService.ts`, `src/services/__tests__/audioStorageService.test.ts` — `deleteStaleProjectAudio()`를 추가해 현재 snapshot에 없는 scene/merged 오디오 blob을 auto-save 시 정리하도록 수정하고 stale audio prune 회귀 테스트 1건 추가. #695, #1053 대응
  - `src/utils/soundToImageBridge.ts` — 업로드 전사 transfer와 일반 TTS transfer를 분리하고, 일반 TTS 라인 수 불일치는 `buildDirectScenesFromLines()`로 직접 재구성하도록 수정. 이미지/영상 탭 이동 시 단락 수가 이전 scene 개수로 축소되던 문제 보정. #689 대응
  - `src/components/tabs/SoundStudioTab.tsx` — scenes/lines/config selector를 구독하도록 바꿔 대본/프로젝트 변경 후에도 narration line 재동기화가 다시 실행되게 수정. #887 대응
  - `src/services/transcriptionService.ts`, `src/utils/uploadedTranscriptScenes.ts`, `src/components/tabs/sound/VoiceStudio.tsx` — `alignTranscriptSegmentsToParagraphs()`와 transcript paragraph 우선 scene 빌더를 추가해 업로드 오디오 전사 결과를 현재 대본 단락 구조에 맞춰 재배열하고 그 결과를 config/script/raw segment에 함께 저장하도록 수정. #728, #1006 대응
  - `src/components/tabs/sound/TypecastEditor.tsx` — 활성 줄 기준 화자 동기화, `handleRegenerateActiveLine()` 추가, 재생 전 `syncEditorToStore()` 강제, `<br>` 보존 텍스트 추출, Mac 복사 `onCopy`, 우측 감정/속도 패널 실측 높이 정렬, speaker speed/emotionPreset 동기화, TTS 다운로드 시 SRT 동시 저장(병합은 별도 SRT, 분할 ZIP에는 `narration.srt` 포함)으로 보강. #720, #773, #817, #851, #861, #880 대응
  - `src/components/tabs/editroom/SceneNarrationPlayer.tsx` — 업로드 전사 라인에 개별 `audioUrl`이 없을 때 `mergedAudioUrl` 구간 재생 fallback을 추가해 편집실에서 외부 전사 나레이션이 비어 보이지 않도록 수정. #1053 대응
  - `src/components/tabs/sound/MusicLibrary.tsx` — 그룹 헤더에 음악 묶음 ZIP 다운로드를 추가하고, 저장된 트랙 오디오를 ZIP으로 묶어 한 번에 내려받도록 보강. #844 대응
  - `cd src && node_modules/typescript/bin/tsc --noEmit`: 통과
  - `cd src && node_modules/.bin/vitest run`: 통과 (`18 passed`, `158 passed`)
  - `cd src && node_modules/.bin/vite build`: 성공 (기존 dynamic import / chunk-size warning만 출력)
  - `rg -n "buildDirectScenesFromLines|alignTranscriptSegmentsToParagraphs|deleteStaleProjectAudio|handleRegenerateActiveLine|downloadSrtForLines|handleDownloadGroupZip|usesMergedFallback" src docs/CHECKLIST.md`: 탭 전송, 전사 정렬, 오디오 정리, Typecast 재생성/SRT, 음악 ZIP, 편집실 merged fallback, 체크리스트 반영 위치 재검증
...
- [x] **2026-04-12 — B-5 NLE 잔여 이슈 3건 점검 및 2건 웹 수정 (#1125 #1123 #962)**
  ...
  - 원인 확인: `src/components/tabs/channel/VideoAnalysisRoom.tsx`, `src/components/tabs/EditRoomTab.tsx`, `src/components/tabs/imagevideo/StoryboardPanel.tsx`가 Premiere ZIP 다운로드 직후 `installNleViaCompanion()`을 계속 호출하고 있었고, `companion/src-tauri/src/server.rs`는 Premiere 타깃에서 `.prproj`를 바로 실행해 사용자가 ZIP을 압축 해제하고 Import 파일 리스트 UI를 거칠 기회를 빼앗고 있었음
  - 조사 결과: #962의 CapCut "비정상 경로"는 현재 웹 코드가 이미 draft path 치환과 설치 스크립트 생성을 수행하고 있고, 실제 오류 지점은 CapCut 신버전 path 검증 또는 companion/native 설치 루트와의 호환성 확인이 필요해 이번 회차에서는 웹 코드를 추측으로 바꾸지 않고 후속 native/실기기 검증 이슈로 남기기로 판단
  - `src/services/nleExportService.ts` — narration 전용 SRT 경로를 audio/timing 기반으로 보강해 `line.text`가 비어도 scene text fallback으로 `_나레이션.srt`를 생성하도록 수정. `buildNlePackageZip()`의 Blob 삽입도 `toJsZipBlobInput(...)` 기반으로 통일해 Node/회귀 테스트 환경의 ZIP 패키징 안정성을 맞춤
  - `src/services/nleExportService.ts` — `EditRoomNarrationLine.text`, `buildEditRoomNarrationSrt()`, narration clip text fallback을 추가하고 `buildEditRoomNleZip()`이 편집실/스토리보드 export에서도 `${safeName}_나레이션.srt`를 함께 넣도록 보강
  - `src/services/nleExportService.ts`, `src/components/tabs/EditRoomTab.tsx`, `src/components/tabs/imagevideo/StoryboardPanel.tsx`, `src/components/tabs/channel/VideoAnalysisRoom.tsx` — `shouldAutoInstallNleViaCompanion()` helper를 추가하고 Premiere는 ZIP 다운로드 후 자동 설치를 건너뛰도록 변경. 사용자 토스트도 "압축 해제 후 File > Import" 수동 흐름 기준으로 정리
  - `src/services/__tests__/nleExportService.premiere2024.test.ts` — `일반 NLE ZIP은 narration line text가 비어 있어도 장면 텍스트로 나레이션 SRT를 생성한다`, `Edit Room NLE ZIP은 scene text fallback으로 나레이션 SRT를 함께 패키징한다` 회귀 테스트 2건 추가
  - `cd src && node_modules/typescript/bin/tsc --noEmit`: 통과
  - `cd src && node_modules/.bin/vitest run services/__tests__/nleExportService.premiere2024.test.ts`: 통과 (`24 passed`)
  - `cd src && node_modules/.bin/vitest run`: 기존 unrelated 실패 2건으로 실패 (`test/useAutoSave.test.tsx`의 `second image persistence retry did not run`, `persistProjectImages called 3 times`) / 이번 NLE 회귀 테스트 파일은 통과
  - `cd src && node_modules/.bin/vite build`: 성공 (기존 dynamic import / chunk-size warning만 출력)
  - `rg -n "shouldAutoInstallNleViaCompanion|buildEditRoomNarrationSrt|_나레이션\\.srt|generateNleSrt\\(scenes, 'narration'" src/services/nleExportService.ts src/components/tabs/EditRoomTab.tsx src/components/tabs/imagevideo/StoryboardPanel.tsx src/components/tabs/channel/VideoAnalysisRoom.tsx src/services/__tests__/nleExportService.premiere2024.test.ts`: narration SRT 보강, Premiere 수동 import 분기, 회귀 테스트 반영 위치 재검증

- [x] **2026-04-12 — Group E+G 실제 버그 5건 동시 수정 (#1130 #1075 #1117 #1116 #1071)**
  - `rg -n "storyboard|스토리보드|sceneIndex|map\\(.*, i.*=>" src/services/gemini/scriptAnalysis.ts src/components/tabs/imagevideo/StoryboardPanel.tsx`, `rg -n "generateStoryboard|generateScene.*Image|sceneImagePrompt" src/services/`, `rg -n "롱폼|long.*form|script.*word|target.*length|maxLength|5000" src/services/geminiService.ts src/components/tabs/ScriptWriterTab*`, `rg -n "generateScript|writeScript|scriptContinue" src/services/geminiService.ts src/components/tabs/ScriptWriterTab*`, `rg -n "ScriptWriterTab|scriptBatch|multipleScripts|scriptCount" src/`, `rg -n "getVideoMetadata|fetchVideoInfo|youtube.*metadata|메타데이터" src/services/`, `rg -n "Promise\\.all|Promise\\.allSettled|versionOffset|versionCount|versionIds" src/components/tabs/channel/VideoAnalysisRoom.tsx src/services/videoAnalysisBatchService.ts`로 스토리보드 장면 매핑, 롱폼 대본 생성/이어쓰기, 배치 버전 정규화, YouTube 메타데이터 경로를 전수 조사
  - 원인 확인: `src/services/gemini/scriptAnalysis.ts`에서 AI가 장면을 누락하거나 합친 경우 후속 scene의 `scriptText`/`visualPrompt`가 단순 index 기준으로 밀리며 후반부 단락 프롬프트 mismatch가 누적될 수 있었음. 또한 `src/components/tabs/channel/VideoAnalysisRoom.tsx`의 배치 응답 정규화는 요청 범위를 벗어난 VERSION이 먼저 들어오면 사용자가 요청한 슬롯을 밀어낼 수 있었고, YouTube 메타데이터 실패는 원인 분류 없이 동일한 "확인 불가"로만 노출되고 있었음
  - `src/services/gemini/scriptAnalysis.ts` — `alignGeneratedScenesToSourceTexts()`와 `remapGeneratedScenesToSourceTexts()`를 추가해 AI 생성 장면을 원본 단락 텍스트 유사도 기준으로 다시 매핑하도록 수정. 청크/비청크 경로 모두 누락 장면은 fallback scene으로 채우고, 저신뢰 visual prompt는 재사용하지 않도록 변경해 후반부 인덱스 shift가 다음 단락으로 전염되지 않게 보강
  - `src/services/scriptGenerationService.ts` 신규, `src/components/tabs/ScriptWriterTab.tsx` — 목표 글자수 기반 동적 `maxOutputTokens` 계산과 공통 이어쓰기 loop를 추가. 초안이 목표의 92% 미만이거나 `MAX_TOKENS`로 끊기면 최대 5회까지 이어서 생성하고, 이어쓰기 실패 시 현재까지 생성된 본문을 보존한 채 사용자에게 이어쓰기 안내를 띄우도록 정리. 프롬프트 본문은 건드리지 않고 호출부 구조만 조정
  - `src/services/videoAnalysisBatchService.ts`, `src/components/tabs/channel/VideoAnalysisRoom.tsx` — `normalizeVideoAnalysisBatchVersions()`와 `getVideoAnalysisBatchVersionIdCandidates()`를 추가해 요청 범위 안의 VERSION ID를 우선 배치하고 overflow/out-of-range 버전이 요청 슬롯을 선점하지 못하도록 수정. 배치 결과는 기존 `runVideoAnalysisBatches()` 경로에서 계속 부분 성공을 유지하며 누락 슬롯만 failedVersions로 남도록 보강
  - `src/services/youtubeVideoMetaService.ts` 신규, `src/components/tabs/channel/VideoAnalysisRoom.tsx` — YouTube 메타데이터 fetch를 공통 서비스로 분리하고 `invalid_video_id`, `quota_exceeded`, `private_or_unavailable`, `forbidden`, `request_failed` 등 세부 오류 코드/메시지를 도입. quota 초과 시 다음 API 키로 회전 재시도하고, UI에는 URL 형식 오류/할당량 초과/비공개 영상 등의 구체적 사유와 URL을 함께 표시하도록 수정
  - `src/services/__tests__/scriptAnalysisAlignment.test.ts` 신규 — 중간 장면이 누락돼도 뒤쪽 단락 프롬프트가 앞으로 당겨지지 않는지 검증
  - `src/services/__tests__/scriptGenerationService.test.ts` 신규 — 목표 분량 미달 시 이어쓰기 재호출, 이어쓰기 실패 시 부분 결과 보존, 동적 토큰 budget 상향을 검증
  - `src/services/__tests__/videoAnalysisBatchService.test.ts` — 요청한 2개 VERSION보다 overflow VERSION 3이 먼저 와도 요청 범위가 우선 유지되는 회귀 테스트 추가
  - `src/services/__tests__/youtubeVideoMetaService.test.ts` 신규 — quota 초과/403 일반 거부 분류와 사용자 메시지 매핑을 검증
  - `cd src && node_modules/typescript/bin/tsc --noEmit`: 통과
  - `cd src && node_modules/.bin/vitest run`: 통과 (`15 passed`, `145 passed`)
  - `cd src && node_modules/.bin/vite build`: 성공 (기존 dynamic import / chunk-size warning만 출력)
  - `rg -n "alignGeneratedScenesToSourceTexts|remapGeneratedScenesToSourceTexts" src/services/gemini/scriptAnalysis.ts`, `rg -n "generateScriptWithContinuations|looksLikeNaturallyFinishedScript|handleScriptContinuationFailure" src/components/tabs/ScriptWriterTab.tsx src/services/scriptGenerationService.ts`, `rg -n "normalizeVideoAnalysisBatchVersions|getVideoAnalysisBatchVersionIdCandidates" src/components/tabs/channel/VideoAnalysisRoom.tsx src/services/videoAnalysisBatchService.ts`, `rg -n "fetchYouTubeVideoMeta|YouTube URL 형식 오류|quota_exceeded|private_or_unavailable" src/components/tabs/channel/VideoAnalysisRoom.tsx src/services/youtubeVideoMetaService.ts`: 정렬 보정, 롱폼 이어쓰기, 배치 정규화, 메타데이터 오류 세분화 반영 위치 재검증
